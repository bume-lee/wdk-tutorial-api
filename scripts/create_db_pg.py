...
def get_conn_string():
    conn_string = f'''host={exec_info['host']} port={exec_info['port']} dbname={exec_info['name']} user={exec_info['user']} password={exec_info['password']}'''
...
def get_data(query):
    ...
    result_data = [
        dict(zip([column.name for column in columns], row)) for row in rows]
...
def create_tables():
    ...
    create_table_script = os.path.join(repository_path, 'scripts', 'create_db_tables_pg.sql')
...
def create_objects():
    ...
    repository_path = exec_info['repository_path']
    create_ojbect_script = os.path.join(repository_path, 'scripts', 'create_db_objects_pg.sql')
...
def insert_data():
    print('기초 데이터를 등록합니다.')
    repository_path = exec_info['repository_path']
    create_data_script = os.path.join(repository_path, 'scripts', 'create_db_data_pg.sql')
